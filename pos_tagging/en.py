...
# Command: python3 en.py <source_file_zh> <result_file_name>
import spacy
import os
import sys
...
PREFERRED_EXTENSION = ".en"
# install vocab model from spacy.io
cache_dir = os.getenv("cache_dir", "../../models")
model_path = SPACY_MODEL_NAME_EN
...
#process tagging
def process(source_file_en, result_file_name):
    sentences = read_file(source_file_en)
...
if __name__ == '__main__':
    source_file_en = sys.argv[1]
    result_file_name = sys.argv[2] if len(sys.argv) > 2 else get_default_result_filename(source_file_en)
    if not source_file_en.endswith(PREFERRED_EXTENSION):
        print("Wrong file extension: " + source_file_en + " Please try another file with đuôi " + PREFERRED_EXTENSION)
        exit(1)
    print("Processing... Source File: " + source_file_en)
    process(source_file_en, result_file_name)